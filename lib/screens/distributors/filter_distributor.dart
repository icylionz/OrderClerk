...
class FilterDistributor extends StatefulWidget {
  ...
  @override
  _FilterDistributorState createState() => _FilterDistributorState();
}

class _FilterDistributorState extends State<FilterDistributor> {
  FToast fToast = FToast();
  late ScrollController _horizontalController;
  late ScrollController _verticalController;
  GlobalKey<FormBuilderState> _distributorFilterForm =
                ...
                child: Row(
                  children: [
                    ...
                    TextButton(
                        onPressed: () {
                          setState(() {
                            _distributorFilterForm.currentState!.save();
                            if (_distributorFilterForm.currentState!
                                .validate()) {
                              submitFilter();
                            }
                          });
              ...
              child: Container(
                  ...
                  child: ListView.separated(
                      itemBuilder: (BuildContext context, int index) {
                        return ListTile(
                          title: Text((DistributorFilter.filter[index]
                                      .name ?? // The field being searched
                                  "null") +
                              " " +
                              (DistributorFilter
                                      .filter[index].isDate // Date Format
                                  ? "between ${DistributorFilter.filter[index].firstDate} and ${DistributorFilter.filter[index].lastDate}"
                                  : ((DistributorFilter.filter[index]
                            ...
                            onPressed: () {
                              setState(() {
                                DistributorFilter.delete(
                                    entry: DistributorFilter.filter[index]);
                                refresh();
                              });
                            },
                          ),
                      ...
                      separatorBuilder: (BuildContext context, int index) =>
                          Divider(),
                      itemCount: DistributorFilter.filter.length)),
            ),
          ],
        ),
      ),
  ...
  void submitFilter() {
    ...
    if (dateSelected)
      DistributorFilter.add(
        name: nameMap["${_distributorFilterForm.currentState!.value["where"]}"],
        where: _distributorFilterForm.currentState!.value["where"],
      );
    else
      DistributorFilter.add(
          name:
              nameMap["${_distributorFilterForm.currentState!.value["where"]}"],
          exact:
              _distributorFilterForm.currentState!.value["criteria"] == "exact"
                  ? true
                  : false,
          contains: _distributorFilterForm.currentState!.value["criteria"] ==
                  "contains"
              ? true
              : false,
          begins:
              _distributorFilterForm.currentState!.value["criteria"] == "begins"
                  ? true
                  : false,
          ends: _distributorFilterForm.currentState!.value["criteria"] == "ends"
              ? true
              : false,
          where: _distributorFilterForm.currentState!.value["where"],
          whereArgs: _distributorFilterForm.currentState!.value[
